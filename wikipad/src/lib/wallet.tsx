...
import { sanitizeUserError } from './safeError.ts'
import {
...
const STORAGE_KEY = 'wikipad.wallet.rdns'
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const connect = useCallback(async (d: ProviderDetail) => {
    ...
    try {
      ...
      setError(err?.code === 4001 ? null : sanitizeUserError(err, 'Could not connect'))
    } finally {
      setConnecting(false)
    }
  }, [attach])
  ...
  const switchAccount = useCallback(async () => {
    ...
    try {
      ...
      const err = e as ProviderRpcError
      setError(err?.code === 4001 ? null : sanitizeUserError(err, 'Could not switch account'))
    ...
    try {
      ...
      if ((e as ProviderRpcError)?.code === 4902) {
        try {
          ...
          if ((addErr as ProviderRpcError)?.code === 4001) return
          const msg = sanitizeUserError(addErr, 'Could not switch network')
          setError(msg)
          throw new Error(msg)
        }
      }
      if ((e as ProviderRpcError)?.code === 4001) return
      const msg = sanitizeUserError(e, 'Could not switch network')
      setError(msg)
      throw new Error(msg)
    }
  }, [active])
