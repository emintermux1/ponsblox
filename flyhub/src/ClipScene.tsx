...
import { seedOf, type ClipKind } from './clips.ts'
...
function fillScene(ctx: CanvasRenderingContext2D, w: number, h: number, kind: ClipKind) {
  switch (kind) {
    case 'night':
...
function setDress(ctx: CanvasRenderingContext2D, w: number, h: number, kind: ClipKind, t: number) {
  switch (kind) {
    case 'fruit': {
      ...
      ctx.beginPath()
      ctx.ellipse(w * 0.4, h * 0.66, w * 0.06, h * 0.04, 0, 0, Math.PI * 2)
      ctx.fill()
      break
    }
    case 'drain': {
    ...
    case 'night': {
      ...
      ctx.fillStyle = 'rgba(255, 176, 60, 0.1)'
      ctx.beginPath()
    ...
    case 'compost': {
      ...
      ctx.beginPath()
      ctx.ellipse(w * 0.64, h * 0.72, w * 0.16, h * 0.08, 0.15, 0, Math.PI * 2)
      ctx.fill()
      break
    }
    case 'swarm': {
      ...
      ctx.strokeStyle = 'rgba(255, 144, 0, 0.28)'
...
export function ClipScene({ kind, seed, detail = 'thumb' }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const w = Math.max(parent?.clientWidth || 160, 160)
      const h = Math.max(parent?.clientHeight || 90, 90)
  ...
  return <canvas ref={ref} className="clip-scene" />
}
