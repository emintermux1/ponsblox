...
function discover(): Eip1193 | undefined {
  ...
  const ranked = [...announced.values()].sort((a, b) => {
    ...
    const ai = pref.findIndex(
      (r) => a.info.rdns === r || a.info.rdns.startsWith(`${r}.`),
    );
    const bi = pref.findIndex(
      (r) => b.info.rdns === r || b.info.rdns.startsWith(`${r}.`),
    );
...
export const injected = createConnector((config) => {
  ...
  return {
    ...
    connect: (async () => {
      ...
      const accounts = (
        (await p.request({ method: "eth_requestAccounts" })) as string[]
