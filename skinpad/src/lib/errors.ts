...
export type LaunchRetry = 'skin' | 'token' | 'review' | 'resume' | 'deploy'
...
export function classifyError(raw: string): AppError {
  ...
  if (/TokenLaunched was not|not marking LIVE|Indexer has not/i.test(m)) {
    return {
      ...
      next: 'The hash is saved. Watch the explorer. SkinPad will not send another launch.',
    }
  }
  ...
  if (/rate limit|429/i.test(m)) {
    return { code: 'RATE_LIMITED', message: 'Steam rate-limited this server.', next: 'Wait a minute, then refresh the catalogue.' }
  }
  if (/Failed to fetch|\bNetworkError\b|\bLoad failed\b|network request failed|API is unreachable/i.test(m)) {
    return {
      ...
      message: 'SkinPad API is unreachable.',
      next: 'Refresh. Local needs the Vite server on port 5178.',
    }
  }
  if (/Steam|catalogue|priceoverview/i.test(m)) {
    return { code: 'STEAM_UNAVAILABLE', message: 'Steam market feed is unreachable.', next: 'Last known prices are held. Retry in a minute.' }
  }
  if (/Pinata|IPFS|PINATA|CID was returned/i.test(m)) {
    return { code: 'IPFS_FAILED', message: m, next: 'Retry, or launch with the short SkinPad art URL.' }
  }
  ...
  if (/skin|catalogue item|hash name/i.test(m) && /not found|unknown/i.test(m)) {
    return { code: 'SKIN_NOT_FOUND', message: 'That skin is not in the catalogue.', next: 'Pick a listing from the catalogue.' }
  }
...
export function launchFailTitle(code: AppErrorCode): string {
  switch (code) {
    case 'STEAM_UNAVAILABLE': return 'Steam market feed is unreachable.'
    case 'SKIN_NOT_FOUND': return 'That skin is not in the catalogue.'
    case 'RATE_LIMITED': return 'Steam rate-limited this server.'
