...
function gitpadApi(): Plugin {
  const run = () => async (req: { method?: string; url?: string }, res: {
    ...
    try {
      const url = new URL(req.url, 'http://gitpad.local')
      ...
      console.error('[gitpad:api]', (e as Error).message)
      res.statusCode = 500
  ...
  return {
    ...
    configureServer(server) {
      if (!process.env.GITHUB_TOKEN) console.warn('[gitpad:boot] GITHUB_TOKEN unset — GitHub lists will rate-limit.')
      if (!process.env.PINATA_JWT) console.warn('[gitpad:boot] PINATA_JWT unset — no IPFS CIDs will be invented.')
      const chain = process.env.VITE_ROBINHOOD_CHAIN_ID
      if (chain && chain !== '4663') console.warn(`[gitpad:boot] VITE_ROBINHOOD_CHAIN_ID=${chain} is not Robinhood 4663.`)
