...
export const MUSE_SPRITES_IN_WORLD = true;
...
export function grokLookAt(world: WorldSnapshot): [number, number, number] {
  const id = world.selected ?? world.grokWake.museId;
  if (id) {
    const position = world.muses[id].position;
    return [position[0], position[1] + 0.74, position[2]];
  }
  const chill = world.muses.chill.position;
  return [chill[0], chill[1] + 0.7, chill[2]];
}

export function grokCompanyLine(museName: string | null): string {
  if (!museName) {
    return "GROK is watching the loft";
  }
  return `GROK is with ${museName}`;
}
