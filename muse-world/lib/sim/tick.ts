    ...
    case "ask_card":
      ...
      return {
        ...
        events: pushEvent(
          events,
          "SOCIAL_REACTION",
          `${muses.trader.name} wants a card on ${ticker}`,
          "trader",
          now,
          random,
        ),
        packet: { from: "trader", to: "builder", label: ticker, t: now, kind: "NOTE" },
        wallPins,
      };
    case "share_builder":
      if (!ticker) {
      ...
      return {
        ...
        events: pushEvent(
          events,
          "SOCIAL_REACTION",
          `${muses.scroller.name} pinged ${muses.builder.name} · ${ticker}`,
          "scroller",
          now,
          random,
        ),
        packet: { from: "scroller", to: "builder", label: ticker, t: now, kind: "NOTE" },
        wallPins,
      };
    case "wave_chill":
      if (!ticker) {
      ...
      return {
        ...
        events: pushEvent(
          events,
          "SOCIAL_REACTION",
          `${muses.scroller.name} waved ${ticker} at the couch`,
          "scroller",
          now,
          random,
        ),
        packet: { from: "scroller", to: "chill", label: ticker, t: now, kind: "NOTE" },
        wallPins,
      };
    case "boredom":
      return {
