...
function robloxpadApi(): Plugin {
  const run = () => async (req: { method?: string; url?: string }, res: {
    ...
    try {
      const url = new URL(req.url, 'http://robloxpad.local')
      ...
      console.error('[robloxpad:api]', (e as Error).message)
      res.statusCode = 500
  ...
  return {
    ...
    configureServer(server) {
      if (!process.env.PINATA_JWT) console.warn('[robloxpad:boot] PINATA_JWT unset — no IPFS CIDs will be invented.')
      const chain = process.env.VITE_ROBINHOOD_CHAIN_ID
      if (chain && chain !== '4663') console.warn(`[robloxpad:boot] VITE_ROBINHOOD_CHAIN_ID=${chain} is not Robinhood 4663.`)
