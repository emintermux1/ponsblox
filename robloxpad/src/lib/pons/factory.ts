...
import { ROBLOXPAD_X_URL } from '../../config/official.ts'
import { parsePegHash } from '../metadata.ts'
...
export async function readToken(address: Address): Promise<TokenRecord | null> {
  ...
  return {
    ...
    skinHash: parsePegHash(desc || info?.[2] || ''),
...
export function validateLaunchDraft(d: LaunchDraft, maxTax: number): string | null {
  ...
  if (!d.website.trim()) return 'Roblox game URL is required'
...
export async function buildLaunchCall(d: LaunchDraft): Promise<LaunchCall> {
  ...
  const params = {
    ...
    socials: {
      twitter: d.twitter.trim() || ROBLOXPAD_X_URL,
...
export function feePermissionFor(input: {
  ...
  if (!recipient || recipient === ZERO.toLowerCase()) {
    return {
      ...
      supported: false,
      permission: null,
      reason: 'This launch has no Pons creatorFeeRecipient.',
    }
  }
  return {
    kind: 'need_creator',
...
export function feeBrief(perm: FeePermission | null): string {
  ...
  switch (perm.kind) {
    case 'transfer_recipient':
      return 'Creator connected — you can move the recipient'
    case 'need_creator':
