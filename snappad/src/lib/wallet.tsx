...
import { ROBINHOOD_CHAIN_ID } from './pons/config.ts'
import {
...
const CHAIN_HEX = `0x${ROBINHOOD_CHAIN_ID.toString(16)}` as const

async function makeWalletClient(account: Address, provider: Eip1193Provider): Promise<WalletClient> {
  // Code-split: keep viem / Pons RPC off the first home paint.
  const [{ createWalletClient, custom }, { rhc }] = await Promise.all([
    import('viem'),
    import('./pons/client.ts'),
  ])
  return createWalletClient({ account, chain: rhc, transport: custom(provider) })
}
const STORAGE_KEY = 'snappad.wallet.rdns'
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const connectMetaMask = useCallback(async () => {
    ...
    if (!mm) {
      const fallback = getProviders()[0] ?? legacyProvider()
      if (!fallback) {
        setError('Install a browser wallet, then refresh.')
        setConnecting(false)
        return
      }
  ...
  const switchChain = useCallback(async () => {
    ...
    try {
      ...
      type KnownSwitch = 4001 | 4902
      const known: KnownSwitch | null = code === 4001 || code === 4902 ? code : null
      if (known === null) {
        setError(err?.message ?? 'Could not switch to Robinhood Chain (4663).')
      } else {
        switch (known) {
          case 4001:
            break
          case 4902:
            try {
              ...
              const add = addErr as ProviderRpcError
              setError(add?.code === 4001 ? null : add?.message ?? 'Could not add Robinhood Chain (4663).')
            }
            break
          default: {
            const _e: never = known
            return _e
          }
        }
      }
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void makeWalletClient(address, active.provider).then((client) => {
  ...
  const value: Ctx = {
    address, chainId, onRightChain: chainId === ROBINHOOD_CHAIN_ID, providers: all,
